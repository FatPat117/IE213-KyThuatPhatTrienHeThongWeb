...
export default function MilestoneEvidenceUploadPage() {
    ...
    const [milestoneId, setMilestoneId] = useState(
        initialMilestoneId > 0 ? initialMilestoneId : 0,
    );
    const [title, setTitle] = useState("Báo cáo tiến độ");
    const [description, setDescription] = useState(
        "Minh chứng tiến độ mốc giải ngân",
    );
    ...
    const uploadEvidence = async (targetMilestoneId: number) => {
        ...
        if (!response.ok || !payload?.success) {
            throw new Error(
                payload?.error || payload?.message || "Upload thất bại",
            );
        }
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!cid) {
                setResultMessage(
                    "Upload thành công nhưng không đọc được CID để ghi blockchain.",
                );
                setFile(null);
                return;
            }
            ...
            try {
                ...
                const chainMessage =
                    chainError instanceof Error
                        ? chainError.message
                        : "Không thể ghi CID lên blockchain";
                setErrorMessage(
                    `Upload IPFS thành công (CID: ${cid}) nhưng ghi blockchain thất bại: ${chainMessage}`,
                );
                return;
            }
            ...
            <main className="mx-auto w-full max-w-3xl px-6 py-10 md:px-10">
                ...
                {!token && (
                    <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                        Bạn chưa đăng nhập ví. Một số campaign có thể từ chối
                        upload nếu thiếu token xác thực.
                    </div>
                )}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <label className="block">
                            ...
                            <input
                                ...
                                value={milestoneId}
                                onChange={(event) =>
                                    setMilestoneId(Number(event.target.value))
                                }
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                                Lưu ý: index mốc trên smart contract thường bắt
                                đầu từ 0.
                            </p>
                        </label>
                        <label className="block">
                        ...
                        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            {previewType === "file" && (
                                <p className="text-xs text-slate-600">
                                    ...
                                    bạn vẫn có thể upload bình thường.
                                </p>
                            )}
                        </div>
                    )}
