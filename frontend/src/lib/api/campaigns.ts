...
export async function getReviewerAggregates(): Promise<ReviewerAggregate[]> {
  ...
  for (const campaign of campaigns) {
    ...
    try {
      const nextTotal = BigInt(existing.totalDisbursedWei || '0') + BigInt(campaign.totalDisbursedWei || '0');
  ...
  return Array.from(aggregates.values()).sort((a, b) => b.campaignCount - a.campaignCount);
}
