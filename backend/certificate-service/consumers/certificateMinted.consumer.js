...
// Consumer for certificate.minted events.
async function startCertificateMintedConsumer() {
    const channel = getChannel();
    if (!channel) {
        console.warn(
            "[certificate-service] RabbitMQ channel is unavailable. Consumer startup skipped.",
        );
        return;
    }
    ...
    console.log(
        `[certificate-service] Consumer is listening on queue: ${QUEUE}`,
    );
    ...
    channel.consume(QUEUE, async (msg) => {
        ...
        try {
            ...
            console.log(
                "[certificate-service] Received certificate.minted event:",
                payload,
            );
            ...
            console.log(
                `[certificate-service] Stored certificate tokenId=${payload.tokenId}`,
            );
            channel.ack(msg);
        } catch (err) {
